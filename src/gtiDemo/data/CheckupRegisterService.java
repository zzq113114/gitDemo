...
@Service
public class CheckupRegisterService {
    private static final Logger log = Logger.getLogger(CheckupRegisterService.class);
    private final String RES_URL = "/checkupRegister/checkupMain.do";
    ...
    public List<TreeObject> getIndexMenu(String empiid, String sorttype, String filtertype, String combName) {
        List<TreeObject> indexList = null;
        try {
            indexList = sortCheckupIndexMenu(empiid, sorttype, filtertype, combName);
    ...
    private List<TreeObject> sortCheckupIndexMenu(String empiid, String sorttype, String filtertype, String combName) {
        List<HashMap<String, Object>> res = new ArrayList<HashMap<String, Object>>();
        Map<String, Object> obj = new HashMap<String, Object>();
        List<TreeObject> ns = new ArrayList<TreeObject>();
        try {
            List<Map<String, String>> phridMap = phridService.getPhrIdsAndOrganCodeByEmpiid(empiid, CommonConsts.OP_EM_HP_MARK.TI_JIAN);
            ...
            for(Map<String,String> tmp:phridMap) {
                ...
                obj.put("combName", combName);
                resTmp = indexRecordMapper.queryCheckupIndex(obj);
            ...
            if (0 < res.size()) {
                ...
                if ("sj".equalsIgnoreCase(sorttype)) {
                ...
                } else if ("combname".equalsIgnoreCase(sorttype)) {
                    String temp_name = "";
                    List<TreeObject> nullList = new ArrayList<TreeObject>();
                    List<TreeObject> tList = new ArrayList<TreeObject>();
                    Map<String, Object> temp = null;
                    ...
                    for (int i = 0; i < res.size(); i++) {
                        temp = res.get(i);
                        tTemp = new TreeObject();
                        if (Common.isEmpty((String) temp.get("combname"))) {
                            setChildTreeObject(temp, tTemp, sdf);
                            nullList.add(tTemp);
                        } else {
                            if (Common.isEmpty(temp_name)) {
                                temp_name = (String) temp.get("combname");
                                setChildTreeObject(temp, tTemp, sdf);
                                tList.add(tTemp);
                            } else {
                                if (temp_name.equals((String) temp.get("combname"))) {
                                    setChildTreeObject(temp, tTemp, sdf);
                                    tList.add(tTemp);
                                } else {
    ...
    private void setChildTreeObject(Map<String, Object> temp, TreeObject tTemp, SimpleDateFormat sdf) {
        ...
        if (Common.isNotEmpty((String) temp.get("combname")) && !"null".equalsIgnoreCase(((String) temp.get("combname")).trim())) {
            transTemp = (String) temp.get("combname");
        } else {
            transTemp = "无套餐名称";
        }
        String checkuptype= (String) temp.get("checkuptype");
        if(Common.isEmpty(checkuptype)){
            checkuptype = "";
        }
        tTemp.setName(sjTemp + "  " +checkuptype + "  " + transTemp);
